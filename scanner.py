...
arg_parser.add_argument('--fileout', type=str, help="--fileout 'C:/CSV/HOSTS_VIRT_withScanned.csv'")
arg_parser.add_argument('--ports',nargs='+', type=int, help="--ports 20 21 22 139 445 80 44")
arg_parser.add_argument('--ports_range',nargs='+', type=int, help="--ports_range 20 25")
arg_parser.add_argument('--ip_range', type=str, help="--ip_range 192.168.10.23-192.168.10.25")
arg_parser.add_argument('--IPcol', type=str, help="--IPcol 'IP' - default 'IP'")
arg_parser.add_argument('--OneHost', type=bool, help="--OneHost True, default False")
arg_parser.add_argument('--Ping', type=bool, help="--Ping True, default False")
...
class scanner:
    def __init__(self,  port_list, file_in, file_out, IP_col, flag_many_host=False, port_range=None, ip_range=None, flag_ping=False):
        self.IP_col = IP_col
    ...
    def __import_file (self):
        ...
        DF = file.read()
        IP_list = DF[self.IP_col].to_list()
        return IP_list
    ...
    def scan (self):
        ...
        if self.flag_ping:
            ...
            print(self.file_out)
            tcp_ports = scann_ports(IP, ports, self.file_out)
            tcp_ports.start_scann()

class scann_ports(scanner):
    
    def __init__ (self, IP, ports, file_out):
        self.IP = IP
        self.ports = ports        
        self.file_out = file_out
...
scan_start = scanner(args.ports, args.filein, args.fileout, args.IPcol, args.OneHost, args.ports_range, args.ip_range, args.Ping)
scan_start.scan()
